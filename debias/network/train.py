...
def integration_v(dt,acc_inter,device,args,k=1,r = None):
    ...
    gravity = torch.tensor([[[0], [0], [- 9.8]]]).repeat((acc_inter.shape[0],1,1)).to(device)
...
def get_inference(network, data_loader, device, epoch,args):
    ...
    """
    targets_all, preds_all, preds_raw_all, preds_cov_all, losses_all = [], [], [], [], []
    network.eval()
    ...
    for bid, (feat, targ,ts_inter,ori_r,_, _) in enumerate(data_loader):
        ...
        preds_raw_all.append(torch_to_numpy(pred_raw))
