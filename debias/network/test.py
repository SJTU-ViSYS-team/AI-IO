...
def compute_metrics_and_plotting(args, net_attr_dict, traj_attr_dict, traj_raw_attr_dict):
    """
    ...
    vel_pred_raw = traj_raw_attr_dict["vel_pred"]
...
def make_plots(args, plot_dict, outdir):
    ...
    vel_pred_raw = plot_dict["vel_pred_raw"]
    ...
    plt.plot(vel_pred_raw[:, 0], vel_pred_raw[:, 1])
    plt.plot(vel_gt[:, 0], vel_gt[:, 1])
    plt.axis("equal")
    plt.legend(["Predicted", "Raw integration", "Ground truth"])
...
def net_test(args):
    ...
    model_path = os.path.join(args.out_dir, "checkpoints", args.model_path)
    ...
    for data in test_list:
        ...
        metrics, plot_dict = compute_metrics_and_plotting(
            args, net_attr_dict, traj_attr_dict, traj_raw_attr_dict
        )
