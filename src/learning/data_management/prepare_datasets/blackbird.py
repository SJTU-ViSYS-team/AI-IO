...
'''
    python src/learning/data_management/prepare_datasets/blackbird.py --config config/blackbird.conf
'''
...
# rotate from body to imu frame(no), from imu to body ??? yes
R_b_i = np.array([
    [0., -1., 0.],
    [1., 0., 0.],
    [0., 0., 1.]])
...
def process_sequence(dataset_dir, seq_name, mode, save_txt):
    ...
    pose_csv = os.path.join(seq_dir, "groundTruthPoses.csv")

    assert os.path.isfile(imu_csv), f"IMU 数据文件缺失：{imu_csv}"
    assert os.path.isfile(pose_csv), f"位姿数据文件缺失：{pose_csv}"

    # 读取 imu 时间范围
    imu_data = pd.read_csv(imu_csv, header=None)
    imu_times = imu_data[0].values
...
def process_and_save_to_hdf5(imu_csv, pose_csv, output_dir, sequence_name, times_config, save_txt=False):
    ...
    imu_data = pd.read_csv(imu_csv, header=None)
    imu_data = imu_data.apply(process_imu_data_row, axis=1)
    pose_data = pd.read_csv(pose_csv, header=None)
    pose_data = pose_data.apply(process_pose_data_row, axis=1)
    # print(imu_data.columns)
    imu_times = imu_data[0].values
    pose_times = pose_data[0].values / 1e6
    ...
    # 插值 IMU 数据 (角速度和加速度)
    interp_gyro = interp1d(imu_times, imu_data[[1, 2, 3]].values, axis=0, fill_value="extrapolate")
    interp_accel = interp1d(imu_times, imu_data[[4, 5, 6]].values, axis=0, fill_value="extrapolate")
    # 插值位姿数据 (位置)
    interp_position = interp1d(pose_times, pose_data[[1, 2, 3]].values, axis=0, fill_value="extrapolate")
    # 插值位姿数据 (姿态 - 四元数)
    rotations = Rotation.from_quat(pose_data[[5, 6, 7, 4]].values)
    slerp = Slerp(pose_times, rotations)
    # 插值位姿数据 (速度)
    interp_velocity = interp1d(pose_times, gt_vel, axis=0, fill_value="extrapolate")
...
def process_imu_data_row(row):
    """
    对imu_data的每行数据做坐标转换
    """
    # NOTE: Air-IO提供的imu_data表头gyro与acc搞反了
    acc_i = np.array([row[4], row[5], row[6]])
    gyro_i = np.array([row[1], row[2], row[3]])
