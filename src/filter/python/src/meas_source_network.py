...
class MeasSourceNetwork:
    ...
    def get_displacement_measurement_model_net(self, net_t_s, net_gyr_w, net_accl_w):
        ...
        # get inference
        dp_learnt, dp_cov_learned = self.net(features_t)
        ...
        # meas_cov = np.eye(3)
        dp_cov_learned[dp_cov_learned < -4] = -4  # exp(2 * -4) =~ 0.00034
        meas_cov = DiagonalParam.vec2Cov(dp_cov_learned).cpu().detach().numpy()[0, :, :]
